# -*- coding: utf-8 -*-
"""WaterGuard App"""# -*- coding: utf-8 -*-
"""WaterGuard App with Accessibility Enhancements"""
...
if lang == "en":
    st.markdown("""
    <div role="region" aria-label="Introduction Section" tabindex="0" style="background: rgba(255, 255, 255, 0.9); padding: 2rem; border-radius: 15px; max-width: 900px; margin: 3rem auto; color: #111; box-shadow: 0 8px 20px rgba(0,0,0,0.15); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
    ...
    st.markdown("""
    <div role="region" aria-label="قسم المقدمة" tabindex="0" style="background: rgba(255, 255, 255, 0.9); padding: 2rem; border-radius: 15px; max-width: 900px; margin: 3rem auto; color: #111; box-shadow: 0 8px 20px rgba(0,0,0,0.15); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; direction: rtl; text-align: right;">
...
if day_usage > high_usage_threshold:
    ...
    if lang == 'en':
        st.sidebar.markdown(f'<div role="alert" aria-live="assertive" style="color:#d32f2f; font-weight:bold;">{alert_text_en}</div>', unsafe_allow_html=True)
    else:
        st.sidebar.markdown(f'<div role="alert" aria-live="assertive" style="color:#d32f2f; font-weight:bold;">{alert_text_ar}</div>', unsafe_allow_html=True)

# ---------- ANOMALIES TABLE ---------- #
df_anomalies = df[df['anomaly'] == 'Anomaly']

if lang == 'en':
    st.markdown('<div role="region" aria-label="Detected Anomalies Section" tabindex="0"><h2>🔍 Detected Anomalies (Possible Leaks or Spikes)</h2></div>')
else:
    st.markdown('<div role="region" aria-label="قسم anomalies المكتشفة" tabindex="0"><h2>🔍 anomalies المكتشفة (تسريبات أو زيادات محتملة)</h2></div>')
...
if lang == 'en':
    st.markdown(f'<div role="region" aria-label="Hourly Water Usage Chart" tabindex="0"><h2>📊 Hourly Water Usage for {selected_day}</h2></div>')
else:
    st.markdown(f'<div role="region" aria-label="مخطط استهلاك المياه الساعي ليوم {selected_day}" tabindex="0"><h2>📊 استهلاك المياه الساعي ليوم {selected_day}</h2></div>')
...
if lang == 'en':
    st.markdown(f"""<div role="img" aria-label="Line chart showing hourly water usage for {selected_day}" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    Line chart shows hourly water usage with anomalies marked in red crosses.
    </div>""", unsafe_allow_html=True)
else:
    st.markdown(f"""<div role="img" aria-label="مخطط خطي يُظهر استهلاك المياه الساعي ليوم {selected_day}" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    يُظهر المخطط تغيّرات استهلاك المياه مع وضع علامات للخلل باللون الأحمر.
    </div>""", unsafe_allow_html=True)

df_daily = df.set_index('timestamp').resample('D')['usage_liters'].sum().reset_index()
if lang == 'en':
    st.markdown('<div role="region" aria-label="Daily Water Usage Chart" tabindex="0"><h2>📈 Daily Water Usage (Past Year)</h2></div>')
else:
    st.markdown('<div role="region" aria-label="مخطط استهلاك المياه اليومي للسنة الماضية" tabindex="0"><h2>📈 استهلاك المياه اليومي (السنة الماضية)</h2></div>')
...
st.pyplot(fig2)
if lang == 'en':
    st.markdown("""<div role="img" aria-label="Line chart showing daily water usage over past year" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    Line chart shows daily water usage totals for each day in the past year.
    </div>""", unsafe_allow_html=True)
else:
    st.markdown("""<div role="img" aria-label="مخطط استهلاك المياه اليومي للسنة الماضية" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    يُظهر المخطط مجموع استهلاك المياه اليومي لكل يوم خلال السنة الماضية.
    </div>""", unsafe_allow_html=True)

df_monthly = df.set_index('timestamp').resample('M')['usage_liters'].sum().reset_index()
if lang == 'en':
    st.markdown('<div role="region" aria-label="Monthly Water Usage Chart" tabindex="0"><h2>📉 Monthly Water Usage (Past Year)</h2></div>')
else:
    st.markdown('<div role="region" aria-label="مخطط استهلاك المياه الشهري للسنة الماضية" tabindex="0"><h2>📉 استهلاك المياه الشهري (السنة الماضية)</h2></div>')
...
st.pyplot(fig3)
if lang == 'en':
    st.markdown("""<div role="img" aria-label="Line chart showing monthly water usage over past year" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    Line chart shows monthly water usage totals for each month in the past year.
    </div>""", unsafe_allow_html=True)
else:
    st.markdown("""<div role="img" aria-label="مخطط استهلاك المياه الشهري للسنة الماضية" style="position:absolute; left:-10000px; top:auto; width:1px; height:1px; overflow:hidden;">
    يُظهر المخطط مجموع استهلاك المياه الشهري لكل شهر خلال السنة الماضية.
    </div>""", unsafe_allow_html=True)

# ---------- FAQ / ADDITIONAL INFO ---------- #
if lang == 'en':
    st.markdown("""
    <div role="region" aria-label="Frequently Asked Questions" tabindex="0" style="max-width: 900px; margin: 3rem auto; padding: 1rem; background: #f0f8ff; border-radius: 12px;">
        <h2>❓ Frequently Asked Questions</h2>
        <h3>How does WaterGuard detect leaks?</h3>
        <p>It uses AI anomaly detection models to spot unusual spikes in water usage that differ significantly from normal patterns.</p>
        <h3>Can I customize my daily water quota?</h3>
        <p>Yes, the daily quota can be adjusted in the app settings to reflect your household needs.</p>
        <h3>Is my data secure?</h3>
        <p>WaterGuard processes data locally and respects your privacy — no data is sent to external servers without permission.</p>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
    <div role="region" aria-label="الأسئلة الشائعة" tabindex="0" style="max-width: 900px; margin: 3rem auto; padding: 1rem; background: #f0f8ff; border-radius: 12px; direction: rtl; text-align: right;">
        <h2>❓ الأسئلة الشائعة</h2>
        <h3>كيف يكشف ووتر جارد التسريبات؟</h3>
        <p>يستخدم نماذج الذكاء الاصطناعي لكشف الانحرافات الكبيرة في استهلاك المياه التي تختلف عن النمط الطبيعي.</p>
        <h3>هل يمكنني تخصيص الحصة اليومية للمياه؟</h3>
        <p>نعم، يمكن تعديل الحصة اليومية في إعدادات التطبيق لتناسب احتياجات منزلك.</p>
        <h3>هل بياناتي آمنة؟</h3>
        <p>يعالج ووتر جارد البيانات محليًا ويحترم خصوصيتك — لا يتم إرسال أي بيانات إلى خوادم خارجية بدون إذن.</p>
    </div>
    """, unsafe_allow_html=True)
